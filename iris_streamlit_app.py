...
class StreamlitApp:
    ...
    # instance method to get the values
    def construct_sidebar(self):
        
        cols = [col for col in features.columns]
        
        st.sidebar.markdown('<p class="header-style"> Iris Data Classification</p>',
    ...
    # instance method to plot a pie chart
    def plot_pie_chart(self, probabilities):
        
        fig = go.Figure(data=[go.Pie(labels=list(iris_data.target_names), values=probabilities[0])])
    ...
    # instance method to construct the app
    def construct_app(self):
        ...
        st.markdown(
            '<p class="header-style"> Iris Data Prediction </p>',
            unsafe_allow_html = True
        )
        ...
        column_2.markdown(
            f'<p class="font-style"> Probability </p>', 
            unsafe_allow_html=True
        )
